...
PACT enregistre l’échappement résiduel. Les systèmes destinataires de courrier émettent déjà des rapports. Les journaux publics de certificats existent déjà parce que les navigateurs ont exigé un journal d’émission. Ce protocole n’invente pas un flux pour demander au monde de le remplir. Il conserve ce qui était déjà jeté.

Courrier et certificats sont des types résiduels d’origine distincte. Ils restent sur le même arbre en ajout seulement, pour que qui vérifie ait une seule racine contre laquelle recalculer. Ils ne sont jamais fusionnés en un score. Une source résiduelle ultérieure peut s’ajouter comme un autre type étiqueté. Chaque type garde son propre préimage. Les applications peuvent interpréter les champs. Ce protocole ne le fera pas.
...
Les feuilles de certificats portent une date de première vue, un émetteur, une fenêtre de validité et une empreinte. Elles partagent l’arbre et l’espace d’index avec les feuilles de courrier. Elles ne sont jamais fusionnées avec elles.
...
Les deux types sont des feuilles keccak256 dans un arbre de Merkle creux. Les bindings ne doivent pas partager la disposition du préimage, pour qu’une feuille de courrier ne puisse pas collisionner avec une feuille de certificat du même nom.

La page publique montre les deux. Elle ne les additionne pas. Elle ne les moyenne pas. Elle ne produit pas de badge. Quiconque construit une application au-dessus peut interpréter les champs. Les fondre en un seul nombre est le choix de cette application, et ce n’est pas ce protocole.
...
La page est ennuyeuse exprès. Depuis combien de temps des déclarants de courrier indépendants confirment le nom. Combien de rapports. De quelles organisations. Quand un certificat couvrant est apparu pour la première fois. Horloges, rapports, organisations, dates de certificats. Pas de score. Pas de badge. Pas de verdict.

## Ce que ce n’est pas

Pas du KYC. Pas une affirmation qu’un nom est légitime. Pas un badge HTTPS. Pas une attestation personnelle. Pas un remplacement des registres ou des bureaux de crédit. Pas un protocole d’authentification de documents — il répond à quel historique confirmé de façon indépendante a été publié pour un domaine, non si un message ou une pièce jointe particulière est authentique.
