...
This document is the single protocol specification. It supersedes the earlier split drafts (v0.1 score / display, v0.2 Merkle / encoding). Scoring is application policy, not protocol — see `docs/examples/scoring.md`.
...
PACT (Provenance of Accumulated Checkable Traces) is an open protocol for establishing verifiable, privacy-preserving chains of independently confirmed domain history from leftover traces already produced by existing infrastructure. The v0.2 binding processes DMARC Aggregate Reports (rua=) — a standard reporting mechanism already generated by every major mail server in the world — to build an append-only Merkle tree of domain-level attestations, all without ever accessing, processing, or storing any message content, recipient identity, or personal data of any kind. PACT does not define a score, an activation label, or a verdict. Applications MAY interpret the published record.
...
PACT provides two guarantees that no existing system provides simultaneously:
...
PACT answers the question: "What independently confirmed authentication history has been published for this domain?" It does not answer: "Is this domain legitimate?" and it does not answer: "Is this specific email or attachment authentic and unaltered?"

Applications that require document-level or message-level proof — legal evidence certification, invoice authentication, individual email verification — can be built on top of PACT, using published history as a cornerstone signal. These are separate products that consume the PACT protocol. They are not part of the PACT protocol itself.
...
**Reporter identity:** The reporting organization is identified by the normalized `org_name` field from `report_metadata`. The envelope SMTP sender is used only for source authentication (Section 3.1.1), not as the reporter identity.
...
Report source authentication prevents fake-report injection and sybil attacks on the published record. A report MUST pass all applicable checks before processing.
...
4. Extract the original `org_name` from report metadata — this is the reporting organization for leaf construction, not the forwarder's domain.
...
#### 3.2.0 Leaf kinds (v0.4)

The Merkle tree is kind-agnostic. Each leaf is a 32-byte keccak256 digest. Bindings MUST NOT share a preimage layout.

| Kind | Tag | Status |
|------|-----|--------|
| DMARC aggregate | none (v0.2 layout, untagged) | Frozen |
...
**Temporal ordering:** Applications and anomaly detection MUST use `period_start` and `period_end` from the report, not ingestion order or tree insertion index. A leaf inserted later with an earlier reporting period attests to the period it records, not its position in the tree.

**High-volume domains:** Domains receiving reports from hundreds of reporters per period produce one leaf per reporter per period. This is expected. Applications that count volume SHOULD use message counts (`dkim_pass_count`), not leaf counts.

#### 3.2.2 Leaf Hash

This encoding is the frozen v0.2 DMARC kind. It is untagged. CT leaves MUST NOT use this preimage.
...
**Sub-commitments (deferred):** v0.2 commits to sorted aggregate hashes for selectors and IPs. A later revision MAY introduce an internal Merkle sub-tree over `(selector, ip_range, pass_count, fail_count)` tuples within a DMARC leaf. That is separate from v0.3 Certificate Transparency leaves (§3.2.3).

#### 3.2.3 Certificate Transparency leaf (v0.3)

CT is leftover exhaust. Public logs exist because browsers required a diary of issuance, not because PACT asked anyone to log. The subject can cause a certificate to be issued. The subject cannot be the log.

The reference ingest reads **crt.sh**, a public *index* over CT logs, not a log operator. Stored `log_id` is `crt.sh`. Stored `log_index` is the crt.sh certificate id. That is weaker than a specific log's STH. Implementations MAY later bind a log's key and signed tree head.

A CT leaf attests first-seen / calendar, not "this domain has HTTPS" and not certificate quality. Let's Encrypt will issue a real certificate in minutes.

**Inclusion:** A CT row MUST cover the connected domain (exact SAN/CN match, or a wildcard whose base is the domain or a parent of it). Rows that only name unrelated SANs MUST be dropped.

**Dedup:** Primary key is `(domain, fingerprint)`. Fingerprint is SHA-256 of the certificate DER when known; otherwise `keccak256(utf8(serial|issuer|notBefore))`.

CT leaves share the same sparse Merkle tree and `leaf_index` space as DMARC leaves. They MUST NOT be merged into DMARC leaves.

See Appendix C.6 for the preimage.

#### 3.2.4 Rekor leaf (v0.4)

Rekor is leftover exhaust. The public log exists because software supply-chain tooling wanted a diary of signed artifacts, not because PACT asked anyone to log. The subject can cause a new signature to be published. The subject cannot be the log.

The reference ingest reads **rekor.sigstore.dev**, a public *index* over that log, not Rekor's own Merkle proof. Stored `log_id` is `rekor.sigstore.dev`. Stored `log_index` is Rekor's `logIndex`. That is weaker than the log's signed tree head. Implementations MAY later bind a log's key and signed tree head.
...
See Appendix C.7 for the preimage.
...
|--------|-----------------|
| Connected domain corpus | A lookalike domain that connects to PACT and accumulates a short independently confirmed history vs. the protected domain |
...
A domain has two distinct ages that PACT MUST never collapse into one number: how long the domain itself has existed, and how long it has been generating independently confirmed PACT history.
...
### 4.3 Interpretation is not protocol
...
Connecting a domain to PACT demonstrates DNS control at connection time. A party with temporary DNS access could add the PACT rua= address and begin accumulating independently confirmed history.
...
Domain Spoofing          Lookalike monitoring (Section 3.4.2).
                         New lookalike has short independently
                         confirmed history and low volume vs.
...
**PACT Proof** — Document-level authentication with explicit user consent. User provides a specific .eml file. Validates DKIM. References published independently confirmed history as corroboration. Separate application specification.
...
**PACT Chain** — Packages Merkle inclusion proofs and independently confirmed history into a portable credential. Separate application specification.
...
- Reference published independently confirmed history as a supplementary signal, not as standalone proof of individual message authenticity.

---

## 9. Smart Contract Specification

The PACT smart contract is minimal by design. It records Merkle roots. It does not interpret history. It does not maintain registries. Anomaly detection, and any scoring an application chooses, is computed off-chain from leaf data verified against on-chain roots.
...
8. **Homograph detection:** Integration of Unicode TR39 confusable detection or external threat feeds.
...
### C.6 CT leaf preimage (v0.3)

```
kind_id = keccak256(utf8("pact-ct-v1"))
domain_hash = keccak256(utf8(normalized_domain))   // C.1
log_id_hash = keccak256(utf8(lower(trim(log_id))))
fingerprint = bytes32  // SHA-256 of DER, or keccak256(utf8(serial|issuer|notBefore))
...
The v0.2 DMARC preimage (C.4) MUST remain untagged. A CT leaf MUST include `kind_id` as the first word so it cannot collide with a DMARC leaf of the same domain.

The reference implementation sets `log_id = "crt.sh"` and `log_index` to the crt.sh numeric id. That indexes public CT; it does not reproduce a specific log's Merkle proof.

### C.7 Rekor leaf preimage (v0.4)
...
preimage = abi.encodePacked(
  bytes32 kind_id,
  bytes32 domain_hash,
  bytes32 entry_id_hash,
  bytes32 identity_hash,
  uint256 integrated_time,
  bytes32 log_id_hash,
  uint256 log_index
)
leaf = keccak256(preimage)
...
| Draft | Role |
|-------|------|
| v0.1 (June 2026) | Historical draft: score formula and display mapping. **Not live protocol.** Informative successor: `docs/examples/scoring.md` (`example-score-0.1`) |
