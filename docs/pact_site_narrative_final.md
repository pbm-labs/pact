# We Build Real — Site manifesto (final)

**Canonical Page 1 copy** — rendered on `/` via `apps/web/src/components/site-narrative.tsx`. Edit here first; do not duplicate elsewhere on the landing page.

The closing CTA line (*"Be one of the first to stand on solid ground."*) lives beneath the button on the homepage, not in this document.
